from crm import User
import pytest
from tinydb import TinyDB, Query
from tinydb.storages import MemoryStorage

@pytest.fixture
def setup_db():
    #On créer une base de donnée en mémoire
    User.DB = TinyDB(storage=MemoryStorage)

@pytest.fixture
def user(setup_db):
    u = User(first_name="John", last_name="Doe", phone_number="+33123456789", address="Paris")
    u.save()
    return u

def test_full_name(user):
    assert user.full_name == "John Doe"

def test_db_instance(user):
    assert isinstance(user.db_instance, table.Document)
    assert user.db_instance["first_name"] == "John"
    assert user.db_instance["last_name"] == "Doe"
    assert user.db_instance["phone_number"] == "+33123456789"
    assert user.db_instance["address"] == "Paris"

def test_not_db_instance(db_instance):
    u = User(first_name="John", last_name="Doe", phone_number="+33123456789", address="Paris")
    assert u.db_instance is None

def test__check_phone_number(setup_db):
    user_good = User(first_name="John", last_name="Doe", phone_number="+33123456789", address="Paris")
    user_bad = User(first_name="Pat", last_name="Doe", phone_number="0123456789", address="Paris")

    #On vérifie que la méthode lève bien une erreur
    with pytest.raises(ValueError) as err:
        user_bad._check_phone_number()

    assert "invalide" in str(err.value)

    user_good.save(validate_data=True)
    assert user_good.exists() is True
            
def test_check_names():
    pass

def test_exists(user):
    assert user.exists() is True

def test_not_exists(setup_db):
    u = User(first_name="John", last_name="Doe", phone_number="+33123456789", address="Paris")
    assert u.exists() is False

def test_delete():
    pass

def test_save():
    pass

